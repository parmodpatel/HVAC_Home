...
import { AuditModal, Closing, Expertise, Hero, Industries, Intro } from './components'
import { SiteFooter } from './Footer'
import { SiteHeader } from './Header'
import { Contact, FAQ, Pricing, Proof, Resources, Testimonials } from './HomepageSections'

const services = [
  { id: 'search', number: '01', title: 'HVAC SEO strategy', text: 'A search strategy built around the services, markets, and high-intent calls that grow your business.' },
  { id: 'local', number: '02', title: 'Local SEO & Maps', text: 'Own the map pack in every service area with optimized locations, profiles, and local authority.' },
  { id: 'content', number: '03', title: 'Content that converts', text: 'Clear, useful pages that answer real homeowner questions and turn more qualified visits into calls.' },
]
...
function App() {
  ...
  return <main className="min-h-screen overflow-hidden bg-[#101817] antialiased">
    <SiteHeader navGroups={navGroups} menuOpen={menuOpen} onMenuToggle={() => setMenuOpen(!menuOpen)} onScrollTo={scrollTo} onOpenAudit={openAudit} />
    <Hero onScrollTo={scrollTo} onOpenAudit={openAudit} />
    <Intro onOpenAudit={openAudit} />
    <Expertise services={services} activeService={activeService} onSelectService={setActiveService} onOpenAudit={openAudit} />
    ...
    <SiteFooter footerColumns={footerColumns} onScrollTo={scrollTo} onOpenAudit={openAudit} />
